...
import { User, Phone, Calendar, Clock, Check, X, Info, Hash } from 'lucide-react';

const AppointmentDetailsModal = ({ isOpen, onClose, item, onChangeStatus ,onCancel}) => {
...
console.log(item)
    ...
    const displayPhone = item.patientPhone || item.guestPhone || "بدون رقم";
            ...
            <div className="bg-white dark:bg-[#0f172a] w-full max-w-md rounded-[2.5rem] shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200 border border-white/10" dir="rtl">
                ...
                <div className="p-6 border-b border-slate-50 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="font-black dark:text-white text-slate-800 text-sm">بيانات الحجز</h2>
                            <p className="text-[10px] text-slate-400 font-bold tracking-widest">ID: #{item.id}</p>
                        </div>
                    </div>
                ...
                <div className="p-6 space-y-4">
                    <div className="space-y-2">
                        <DetailRow label="اسم المريض" value={displayName} icon={<User size={14} />} />
                        <DetailRow label="رقم الهاتف" value={displayPhone} isLtr={true} icon={<Phone size={14} />} />
                        <DetailRow
                            label="التاريخ"
                    ...
                    {item.cancellationReason && (
                        <div className="p-4 rounded-2xl bg-rose-500/5 border border-rose-500/10">
                            <span className="text-[10px] font-black text-rose-500 block mb-1 uppercase">سبب الإلغاء</span>
                    ...
                    <div className="flex flex-col gap-2 pt-4">
                        ...
                        {(isAccepted || isPending) && (
                            <button
                                onClick={() => {onCancel.mutate(item.id) }}
...
// مكون فرعي صغير لتنظيم الصفوف داخل المودال
const DetailRow = ({ label, value, isLtr, icon }) => (
    <div className="flex justify-between items-center p-3.5 bg-slate-50 dark:bg-white/[0.03] rounded-2xl border border-slate-100 dark:border-white/5">
        ...
        <span className={`text-sm font-black text-slate-700 dark:text-slate-200 ${isLtr ? 'tracking-wider' : ''}`} dir={isLtr ? 'ltr' : 'rtl'}>
            {value}
        </span>
    </div>
);
