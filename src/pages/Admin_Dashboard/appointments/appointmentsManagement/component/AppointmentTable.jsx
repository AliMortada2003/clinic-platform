...
import {
    ...
    Eye, Loader2, AlertCircle, User, Stethoscope
} from 'lucide-react';
    ...
    return (
        <div className="overflow-x-auto rounded-[1rem] border border-slate-100 dark:border-white/5 bg-white dark:bg-[#0f172a] shadow-sm">
            <table className="w-full text-right border-collapse p-2 table-fixed min-w-[1000px]">
                <thead className="bg-cyan-600  dark:bg-cyan-800">
                    <tr className="text-white dark:text-slate-50 md:text-[15px] font-black uppercase tracking-widest border-b border-slate-300 dark:border-white/40">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                    {appointments.map((item) => {
                        // بما أن الدكتور واحد، البيانات دائماً تعرض اسم المريض ورقم هاتفه للطرفين
                        const displayName = (item.guestName?.trim() || item.patientName?.trim() || "مريض مجهول");

                        const style = getAvatarStyle(displayName[0]);
                        ...
                        ${item.status === 'Accepted' ? 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-500/20 dark:border-emerald-500/30' :
                                        ...
                                        {item.status === 'Accepted' ? 'مؤكد' : item.status === 'Canceled' ? 'ملغي' : 'قيد الانتظار'}
                                    </span>
                                </td>
                                    ...
                                    <div className="flex justify-center gap-2">
                                        {!isPatientView && isPending ? (
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {/* المريض يستطيع إلغاء موعده فقط إذا لم يكن ملغياً بالفعل */}
                                                {isPatientView && item.status !== 'Canceled' && (
                                                    <button
                                                        onClick={() => onCancel(item.id)}
