              ...
              <div className="relative" ref={menuRef}>
                ...
                <AnimatePresence>
                  {showUserMenu && (
                    ...
                    >
                      {(userRole === 'Doctor') && (
                        <Link to="/admin" className="flex items-center gap-3 px-4 py-3 text-sm text-cyan-600 dark:text-cyan-400 font-bold hover:bg-cyan-50 dark:hover:bg-cyan-500/10 transition-colors">
                          <LayoutDashboard size={18} /> لوحة التحكم
                        </Link>
                      )}

                      <Link to={userRole === "Doctor" ? "/admin/profile" : "/patient/profile"} className="flex items-center gap-3 px-4 py-3 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
