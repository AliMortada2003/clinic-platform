...
const UserMenu = ({ isOnline, goTo }) => {
    ...
    const name = displayName || user?.profile?.name || "مستخدم";
        ...
        <div className="relative" ref={userRef}>
            ...
            {isDropdownOpen && (
                <div className="absolute left-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden py-2 animate-in fade-in zoom-in-95 duration-200">
                    <div className="px-4 py-2 border-b mb-1 text-right">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">الحساب</p>
                        <p className="text-xs font-bold text-slate-800 truncate">{email}</p>
                    </div>

                    <button
                        type="button"
                        onClick={() => { setIsDropdownOpen(false); goTo("profile"); }}
